...
static gint32 load_image(const gchar *filename, gboolean interactive,
                         GError **error);
static gboolean save_image(const gchar *filename, gint32 image_ID,
                           gint32 drawable_ID, gint32 orig_image_ID,
                           GError **error);

static void read_frame(gint32 layer, int bpp, int empty, int trns,
                       guchar *alpha, png_structp pp, png_infop info,
                       png_uint_32 frame_width, png_uint_32 frame_height,
                       png_uint_32 frame_x_offset, png_uint_32 frame_y_offset,
                       GError **error);
static gboolean
write_frame(gint32 drawable_ID, gint bpp, guchar red, guchar green, guchar blue,
            guchar *remap, gboolean as_animation, png_structp pp,
            png_infop info, png_uint_32 offx, png_uint_32 offy,
            png_uint_16 frame_delay_num, png_uint_16 frame_delay_den,
            png_byte frame_dispose_op, png_byte frame_blend_op, GError **error);
#if defined(PNG_APNG_SUPPORTED)
static void parse_delay_tag(png_uint_16 *delay_num, png_uint_16 *delay_den,
                            const gchar *str);
...
static gboolean save_dialog(gint32 image_ID, gboolean alpha);
...
static gint find_unused_ia_color(GimpDrawable *drawable, gint *colors);
...
static void load_gui_defaults(PngSaveGui *pg);
     ...
     (GIMP_MAJOR_VERSION == 2 && GIMP_MINOR_VERSION < 7))
...
static const PngSaveVals defaults = {FALSE,
                                     TRUE,
                                     FALSE,
                                     FALSE,
                                     TRUE,
...
#define FULL_CONFIG_ARGS                                                       \
  OLD_CONFIG_ARGS, {GIMP_PDB_INT32, "comment", "Write comment?"},              \
      {GIMP_PDB_INT32, "svtrans", "Preserve color of transparent pixels?"}

  static const GimpParamDef save_args[] = {COMMON_SAVE_ARGS, OLD_CONFIG_ARGS};

  static const GimpParamDef save_args2[] = {COMMON_SAVE_ARGS, FULL_CONFIG_ARGS};

  static const GimpParamDef save_args_defaults[] = {COMMON_SAVE_ARGS};

  static const GimpParamDef save_get_defaults_return_vals[] = {
      FULL_CONFIG_ARGS};

  static const GimpParamDef save_args_set_defaults[] = {FULL_CONFIG_ARGS};
  ...
  gimp_install_procedure(
      LOAD_PROC, "Loads files in APNG file format",
      "This plug-in loads Animated Portable Network Graphics "
      "(APNG) files.",
      ...
      "Piotr D. Kaczorowski <piotr.kaczorowski@gmai.com",
      PLUG_IN_VERSION, N_("APNG image"), NULL, GIMP_PLUGIN,
      G_N_ELEMENTS(load_args), G_N_ELEMENTS(load_return_vals), load_args,
      load_return_vals);

  gimp_register_file_handler_mime(LOAD_PROC, "image/apng");
  gimp_register_magic_load_handler(LOAD_PROC, "apng", "",
                                   "0,string,\211PNG\r\n\032\n");

  gimp_install_procedure(
      SAVE_PROC, "Saves files in APNG file format",
      "This plug-in saves Animated Portable Network Graphics "
      "(APNG) files.",
      "Daisuke Nishikawa <daisuken@users.sourceforge.net>, "
      "Michael Sweet <mike@easysw.com>, "
      "Daniel Skarda <0rfelyus@atrey.karlin.mff.cuni.cz>",
      ...
      "Daniel Skarda <0rfelyus@atrey.karlin.mff.cuni.cz>, "
      "Nick Lamb <njl195@zepler.org.uk>"
      "Piotr D. Kaczorowski <piotr.kaczorowski@gmail.com",
      PLUG_IN_VERSION, N_("APNG image"), "RGB*,GRAY*,INDEXED*", GIMP_PLUGIN,
      G_N_ELEMENTS(save_args), 0, save_args, NULL);

  gimp_install_procedure(
      SAVE2_PROC, "Saves files in APNG file format",
      "This plug-in saves Animated Portable Network Graphics "
      "(APNG) files. "
  ...
  gimp_install_procedure(
      SAVE_DEFAULTS_PROC, "Saves files in APNG file format",
      "This plug-in saves Animated Portable Network Graphics (APNG) "
      ...
      "Piotr D. Kaczorowski <piotr.kaczorowski@gmail.com>",
      PLUG_IN_VERSION, N_("APNG image"), "RGB*,GRAY*,INDEXED*", GIMP_PLUGIN,
      G_N_ELEMENTS(save_args_defaults), 0, save_args_defaults, NULL);

  gimp_register_file_handler_mime(SAVE_DEFAULTS_PROC, "image/apng");
  gimp_register_save_handler(SAVE_DEFAULTS_PROC, "apng", "");
  ...
  gimp_install_procedure(GET_DEFAULTS_PROC,
                         ...
                         "defaults stored as a parasite for the APNG "
                         "save plug-in. "
                         ...
                         "Nick Lamb <njl195@zepler.org.uk>"
                         "Piotr D. Kaczorowski <piotr.kaczorowski@gmail.com>",
                         PLUG_IN_VERSION, NULL, NULL, GIMP_PLUGIN, 0,
                         G_N_ELEMENTS(save_get_defaults_return_vals), NULL,
                         save_get_defaults_return_vals);

  gimp_install_procedure(SET_DEFAULTS_PROC,
                         "Set the current set of defaults used by the "
                         "APNG file save plug-in",
                         "This procedure set the current set of defaults "
                         "stored as a parasite for the APNG save plug-in. "
                         "These defaults are used to seed the UI, by the "
                         "file_png_save_defaults procedure, and by "
                         "gimp_file_save when it detects to use PNG.",
                         "Daisuke Nishikawa <daisuken@users.sourceforge.net>, "
                         ...
                         "Nick Lamb <njl195@zepler.org.uk>"
                         "Piotr D. Kaczorowski <piotr.kaczorowski@gmail.com>",
                         PLUG_IN_VERSION, NULL, NULL, GIMP_PLUGIN,
...
static void run(const gchar *name, gint nparams, const GimpParam *param,
  ...
  *return_vals = values;
  ...
  if (strcmp(name, LOAD_PROC) == 0) {
  ...
  } else if (strcmp(name, SAVE_PROC) == 0 || strcmp(name, SAVE2_PROC) == 0 ||
    ...
    case GIMP_RUN_NONINTERACTIVE:
      ...
      if (nparams != 5) {
        if (nparams != 12 && nparams != 14) {
          ...
          if (pngvals.compression_level < 0 || pngvals.compression_level > 9) {
            status = GIMP_PDB_CALLING_ERROR;
          }
        }
      }
    ...
    case GIMP_RUN_WITH_LAST_VALS: {
      ...
      export = gimp_export_image(&image_ID, &drawable_ID, NULL, capabilities);
...
static void on_read_error(png_structp png_ptr, png_const_charp error_msg) {
  ...
  for (begin = error_data->begin + error_data->tile_height,
    ...
    gimp_pixel_rgn_set_rect(error_data->pixel_rgn, error_data->pixel, 0, begin,
  ...
  if (png_get_valid(pp, info, PNG_INFO_pHYs)) {
    ...
    if (png_get_pHYs(pp, info, &xres, &yres, &unit_type)) {
      ...
      case PNG_RESOLUTION_METER:
        gimp_image_set_resolution(image, (gdouble)xres * 0.0254,
                                  (gdouble)yres * 0.0254);
        break;
  ...
  if (png_get_valid(pp, info, PNG_INFO_acTL)) {
    ...
    for (frame = 0; frame < num_frames; frame++) {
      ...
      if (png_get_valid(pp, info, PNG_INFO_fcTL)) {
        ...
        png_get_next_frame_fcTL(pp, info, &frame_width, &frame_height,
      ...
      if (frame == 0) {
        if (delay < 0)
          ...
          framename = g_strdup_printf(_("Background (%d%s)"), delay, "ms");
        ...
        gimp_progress_set_text_printf(_("Opening '%s' (frame %d)"),
                                      gimp_filename_to_utf8(filename), frame);
        gimp_progress_pulse();
        ...
        if (delay < 0)
          ...
          framename =
              g_strdup_printf(_("Frame %d (%d%s)"), frame + 1, delay, "ms");
      }
      ...
      gimp_layer_translate(layer, (gint)frame_x_offset, (gint)frame_y_offset);

      read_frame(layer, bpp, empty, trns, alpha, pp, info, frame_width,
                 frame_height, frame_x_offset, frame_y_offset, error);
    }
  } else
#endif
  {
    ...
    layer = gimp_layer_new(
        image, _("Background"), png_get_image_width(pp, info),
        png_get_image_height(pp, info), layer_type, 100, GIMP_NORMAL_MODE);
    gimp_image_add_layer(image, layer, 0);
    ...
    read_frame(layer, bpp, empty, trns, alpha, pp, info,
               png_get_image_width(pp, info), png_get_image_height(pp, info), 0,
               0, error);
  }
...

  {
    ...
    if (png_get_iCCP(pp, info, &profname, &profcomp, &profile, &proflen)) {
      ...
      parasite = gimp_parasite_new(
          "icc-profile", GIMP_PARASITE_PERSISTENT | GIMP_PARASITE_UNDOABLE,
          proflen, profile);
      ...
      if (profname) {
        gchar *tmp = g_convert(profname, strlen(profname), "ISO-8859-1",
...
static void read_frame(gint32 layer, int bpp, int empty, int trns,
  ...
  for (pass = 0; pass < num_passes; pass++) {
    ...
    for (begin = 0, end = tile_height; begin < frame_height;
      ...
      gimp_pixel_rgn_set_rect(&pixel_rgn, pixel, 0, begin, drawable->width,
                              num);

      memset(pixel, 0, tile_height * frame_width * bpp);

      gimp_progress_update(
          ((gdouble)pass + (gdouble)end / (gdouble)frame_height) /
          (gdouble)num_passes);
    }
  }
  ...
  if (trns) {
    ...
    for (begin = 0, end = tile_height; begin < drawable->height;
         begin += tile_height, end += tile_height) {
...
static gboolean save_image(const gchar *filename, gint32 image_ID,
  ...
  if (pngvals.comment) {
    ...
    if (parasite) {
      ...
      text->text = g_convert(comment, -1, "ISO-8859-1", "UTF-8", NULL,
                             &text_length, NULL);
  ...
  if (g_str_has_suffix(filename, ".apng")) {
    new_filename = g_strdup(filename);
  } else {
    new_filename = g_strconcat(filename, ".apng", NULL);
  }
  ...
  case GIMP_INDEXED_IMAGE:
    ...
    pngg.has_plte = TRUE;
    pngg.palette =
        (png_colorp)gimp_image_get_colormap(image_ID, &pngg.num_palette);
...
#if defined(PNG_iCCP_SUPPORTED)
  {
    ...
    if (profile_parasite) {
      GimpParasite *parasite =
          gimp_image_get_parasite(orig_image_ID, "icc-profile-name");
      if (parasite)
  ...
  if (nlayers > 1) {
    for (i = nlayers - 1; i >= 0; i--) {
      ...
      write_frame(layers[i], bpp, red, green, blue, remap, TRUE, pp, info, offx,
                  offy, frame_delay_num, frame_delay_den, frame_dispose_op,
                  frame_blend_op, error);
    }
  } else
#endif
  {
    write_frame(layers[0], bpp, red, green, blue, remap, FALSE, pp, info, offx,
                offy, 0, 0, 0, 0, error);
  }
...
static gboolean write_frame(gint32 drawable_ID, gint bpp, guchar red,
                            guchar green, guchar blue, guchar *remap,
                            gboolean as_animation, png_structp pp,
                            png_infop info, png_uint_32 offx, png_uint_32 offy,
                            png_uint_16 frame_delay_num,
                            png_uint_16 frame_delay_den,
                            png_byte frame_dispose_op, png_byte frame_blend_op,
                            GError **error) {
  ...
  if (as_animation) {
    ...
    png_write_frame_head(pp, info, (png_bytepp)&pixel, drawable->width,
                         drawable->height, frame_x_offset, frame_y_offset,
                         frame_delay_num, frame_delay_den, frame_dispose_op,
                         frame_blend_op);
  }
#endif
  ...
  for (pass = 0; pass < num_passes; pass++) {
    ...
    for (begin = 0, end = tile_height; begin < drawable->height;
      ...
      gimp_progress_update(
          ((double)pass +
           (double)end / (double)png_get_image_height(pp, info)) /
          (double)num_passes);
    }
  }
...
static gboolean ia_has_transparent_pixels(GimpDrawable *drawable) {
  ...
  for (pr = gimp_pixel_rgns_register(1, &pixel_rgn); pr != NULL;
...
static GtkWidget *toggle_button_init(GtkBuilder *builder, const gchar *name,
...
static gboolean save_dialog(gint32 image_ID, gboolean alpha) {
  ...
  g_signal_connect(dialog, "response", G_CALLBACK(save_dialog_response), &pg);
  g_signal_connect(dialog, "destroy", G_CALLBACK(gtk_main_quit), NULL);

  /* GtkBuilder init */
  builder = gtk_builder_new();
  ui_file = g_build_filename(DATADIR, "ui/plug-ins/plug-in-file-apng.ui", NULL);

  if (!gtk_builder_add_from_file(builder, ui_file, &error)) {
    gchar *display_name = g_filename_display_name(ui_file);

    g_printerr(_("Error loading UI file '%s': %s"), display_name,
  ...
  /* Toggles */
  pg.interlaced = toggle_button_init(builder, "interlace", pngvals.interlaced,
                                     &pngvals.interlaced);
  pg.bkgd = toggle_button_init(builder, "save-background-color", pngvals.bkgd,
                               &pngvals.bkgd);
  pg.gama =
      toggle_button_init(builder, "save-gamma", pngvals.gama, &pngvals.gama);
  pg.offs = toggle_button_init(builder, "save-layer-offset", pngvals.offs,
                               &pngvals.offs);
  pg.phys = toggle_button_init(builder, "save-resolution", pngvals.phys,
                               &pngvals.phys);
  pg.time = toggle_button_init(builder, "save-creation-time", pngvals.time,
                               &pngvals.time);
#if defined(PNG_APNG_SUPPORTED)
  pg.as_animation = toggle_button_init(
      builder, "as-animation", pngvals.as_animation, &pngvals.as_animation);
  pg.first_frame_is_hidden = toggle_button_init(
      builder, "first-frame-is-hidden", pngvals.first_frame_is_hidden,
  ...
  pg.comment =
      toggle_button_init(builder, "save-comment",
                         pngvals.comment && parasite != NULL, &pngvals.comment);
  gtk_widget_set_sensitive(pg.comment, parasite != NULL);
  gimp_parasite_free(parasite);

  /* Transparent pixels toggle */
  pg.save_transp_pixels = toggle_button_init(
      builder, "save-transparent-pixels", alpha && pngvals.save_transp_pixels,
...
#endif
               ,
               &tmpvals.interlaced, &tmpvals.bkgd, &tmpvals.gama, &tmpvals.offs,
               &tmpvals.phys, &tmpvals.time, &tmpvals.comment,
               &tmpvals.save_transp_pixels, &tmpvals.compression_level
#if defined(PNG_APNG_SUPPORTED)
               ,
               &tmpvals.as_animation, &tmpvals.first_frame_is_hidden,
               &tmpvals.num_plays, &tmpvals.delay_num, &tmpvals.delay_den,
               &tmpvals.dispose_op, &tmpvals.blend_op
#endif
        );
...
static void save_defaults(void) {
  ...
  def_str = g_strdup_printf(
      "%d %d %d %d %d %d %d %d %d"
#if defined(PNG_APNG_SUPPORTED)
      " %d %d %u %u %u %u %u"
#endif
      ,
      pngvals.interlaced, pngvals.bkgd, pngvals.gama, pngvals.offs,
      pngvals.phys, pngvals.time, pngvals.comment, pngvals.save_transp_pixels,
      pngvals.compression_level
#if defined(PNG_APNG_SUPPORTED)
      ,
      pngvals.as_animation, pngvals.first_frame_is_hidden, pngvals.num_plays,
      pngvals.delay_num, pngvals.delay_den, pngvals.dispose_op, pngvals.blend_op
#endif
  );

  parasite = gimp_parasite_new(PNG_DEFAULTS_PARASITE, GIMP_PARASITE_PERSISTENT,
...
#undef SET_ACTIVE
  ...
  gtk_adjustment_set_value(pg->compression_level, pngvals.compression_level);
#if defined(PNG_APNG_SUPPORTED)
  gtk_adjustment_set_value(GTK_ADJUSTMENT(pg->num_plays), pngvals.num_plays);
#endif
}
